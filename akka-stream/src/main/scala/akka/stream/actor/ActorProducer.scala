...
object ActorProducer {
  ...
  private[akka] object Internal {
    case class Subscribe[T](subscriber: Subscriber[T])
 ...
 * [[#isPreActive]], [[#isActive]], [[#isCompleted]], [[#isErrorEmitted]], and [[#isCanceled]].
 *
...
trait ActorProducer[T] extends Actor {
  ...
  /**
   * The state before the subscriber is attached. It is allowed to
   * call [[#onComplete]] and [[#onError]] in this state, but not
   * [[#onNext]].
   */
  final def isPreActive = lifecycleState == PreSubscriber

  /**
   * The state when an subscriber is attached. It is allowed to
   * call [[#onComplete]] and [[#onError]] in this state. It is
   * allowed to call [[#onNext]] in this state when [[#totalDemand]]
   * is greater than zero.
   */
  final def isActive = lifecycleState == Active
   ...
   */
  final def isCompleted: Boolean = lifecycleState == Canceled

  /**
   * The terminal state after calling [[#onError]]. It is not allowed to
   * call [[#onNext]], [[#onError]], and [[#onComplete]] in this state.
   */
  final def isErrorEmitted: Boolean = lifecycleState match {
    case _: ErrorEmitted ⇒ true
    case _               ⇒ false
  }
